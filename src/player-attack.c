...
static void unarmed_get_punch(struct attack_roll *aroll)
{
	...
	aroll->to_hit = get_power_scale(player, PP_UNARMED_STRIKE, 25) - 10;
	...
	aroll->message = "punch";
	my_strcpy(aroll->name, "punch", sizeof(aroll->name));
	aroll->obj = NULL;
...
static void unarmed_get_kick(struct attack_roll *aroll)
{
	...
	aroll->message = "kick";
	my_strcpy(aroll->name, "kick", sizeof(aroll->name));
	aroll->obj = NULL;
	aroll->proj_type = PROJ_BLUDGEONING;
	aroll->range = 1;
}
...
static void specialization_mod_attack(struct attack_roll *aroll, struct object *obj)
{
	...
	else if (kf_has(obj->kind->kind_flags, KF_SHOOTS_BOLTS)) spec = PP_CROSSBOW_SPECIALIZATION;
												...
												struct object *shooter)
{
	...
	aroll.to_hit = object_to_hit(shooter);
								...
								struct attack_roll *aroll)
{
	...
	aroll->to_hit += object_to_hit(ammo);
...
static void get_thrown_ranged_attack(struct player *p, struct object *thrown, struct attack_roll *aroll)
{
	...
	aroll->ddice = thrown->dd;
	aroll->dsides = thrown->ds;
	aroll->to_hit = object_to_hit(thrown);
							   ...
							   int aind, bool ranged)
{
	...
	if (mb->method->unarmed) {
		...
		memset(aroll, 0, sizeof(*aroll));
		my_strcpy(aroll->name, mb->method->desc, sizeof(aroll->name));
						  ...
						  const struct hit_types *hit_types, int num_types)
{
	...
	p->upkeep->energy_use = z_info->move_energy * 100 / aroll->blows;
...
void do_cmd_throw(struct command *cmd) {
	...
	get_thrown_ranged_attack(player, obj, &aroll);
