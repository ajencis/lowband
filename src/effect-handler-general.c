...
bool effect_handler_CREATE_WALL(effect_handler_context_t *context)
{
	...
	square_set_feat(cave, target, FEAT_GRANITE, 100);
...
bool effect_handler_CREATE_ILLUSORY_WALL(effect_handler_context_t *context)
{
	...
	square_memorize_feat_real(player, cave, target, FEAT_ILLUSORY_WALL);
...
bool effect_handler_ECHOLOCATE(effect_handler_context_t *context)
{
	...
	for (i = 0; i < locsnum; ++i) {
		...
		for (feat = square_feat(cave, grid); feat; feat = feat->next) {
			if (one_in_(3)) {
				square_memorize_feat_real(player, cave, grid, feat->kind->fidx);
			}
		}
	}
