...
static int blow_color(struct player *p, int blow_idx)
{
	...
	if (streq(blow->effect_type, "theft")) {
		...
		int light_slot = slot_by_type(&p->mon, EQUIP_LIGHT, true);
		struct object *obj = slot_object(&p->mon, light_slot);
						   ...
						   bitflag known_flags[RF_SIZE])
{
	...
	if (lore->armour_known) {
		...
		random_chance c;
		struct attack *atk = player->mon.atk;
		hit_chance(&c, atk->to_hit, race->ac);
