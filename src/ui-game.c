...
struct cmd_info cmd_item[] =
{
	...
	{ "Use an item", { 'X' }, CMD_USE, NULL, NULL, 0, NULL, NULL, NULL, 0 }
};
...
{
	{ "Browse a book", { 'b', 'P' }, CMD_BROWSE_SPELL, textui_gener_spell_browse, NULL, 0, NULL, NULL, NULL, 0 },
...
void play_game(enum game_mode_type mode)
{
	while (1) {
		...
		while (!player->is_dead && player->upkeep->playing) {
			pre_turn_refresh();
			// L: possible that queue has been added to
			if (cmdq_is_empty()) {
