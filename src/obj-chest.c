...
static void chest_death(struct loc grid, struct object *chest)
{
	...
	while (number > 0) {
		...
		treasure = make_object(cave, level, large && one_in_(5), false, false, NULL, 0);
		if (!treasure)
			continue;
...
static void chest_trap(struct object *obj)
{
	...
	for (trap = chest_traps; trap; trap = trap->next) {
		if (trap->pval & traps) {
			...
			if (trap->effect) {
				effect_do(trap->effect, source_chest_trap(trap), source_none(), obj, &ident,
						  false, 0, 0, 0, NULL);
			}
			if (trap->destroy) {
				obj->pval = 0;
...
bool do_cmd_open_chest(struct loc grid, struct object *obj)
{
	...
	if (obj->pval > 0) {
		...
		if (player->mon.m_timed[TMD_CONFUSED] || player->mon.m_timed[TMD_IMAGE]) i = i / 10;
