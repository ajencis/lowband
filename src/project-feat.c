...
static void project_feature_handler_FIRE(project_feature_handler_context_t *context)
{
	...
	if ((context->dam > randint1(1800) + 600) &&
		...
		square_set_feat_size(cave, context->grid, FEAT_LAVA, context->dam / 10);
		if (cave->depth == 0) {
	...
	burn_square(cave, context->grid, context->dam, FEAT_FIRE);
}
...
static void project_feature_handler_COLD(project_feature_handler_context_t *context)
{
	...
	if ((context->dam > randint1(900) + 300) &&
		...
		if (one_in_(2)) {
			...
			square_set_feat_size(cave, context->grid, FEAT_RUBBLE, randint1(amt));
		} else {
			square_set_feat_size(cave, context->grid, FEAT_PASS_RUBBLE, randint1(amt));
		}
		if (cave->depth == 0) {
			expose_to_sun(cave, context->grid, is_daytime());
		}
...
static void project_feature_handler_MON_POIS(project_feature_handler_context_t *context)
{
}
...
static void project_feature_handler_CHARM_UNDEAD(project_feature_handler_context_t *context)
{
}
